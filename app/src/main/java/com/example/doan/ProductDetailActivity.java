...
public class ProductDetailActivity extends AppCompatActivity {
    ...
    private BigDecimal basePrice; // Giá gốc của sản phẩm (size S)
    private BigDecimal displayPrice; // Giá hiển thị (sẽ thay đổi theo size)
    ...
    private void updatePriceAndDisplay() {
        ...
        if (selectedSize.equals("M")) {
            displayPrice = basePrice.add(new BigDecimal("5000")); // Size M: +5,000 VNĐ
        } else if (selectedSize.equals("L")) {
            displayPrice = basePrice.add(new BigDecimal("10000")); // Size L: +10,000 VNĐ
        }
    ...
    private void addToCart() {
        ...
        int qty = quantity;
        BigDecimal price = displayPrice; // Giá đã điều chỉnh theo size
