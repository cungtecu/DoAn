...
public class Admin_Product_Add_Activity extends AppCompatActivity implements ProductAdapter.OnProductClickListener {
        ...
        call.enqueue(new Callback<List<Category>>() {
            ...
            public void onResponse(Call<List<Category>> call, Response<List<Category>> response) {
                if (response.isSuccessful()) {
                    ...
                    CategoryAdapter adapter = new CategoryAdapter(Admin_Product_Add_Activity.this, categoryList);
                    spinnerCategory.setAdapter(adapter);
                } else {
                    Toast.makeText(Admin_Product_Add_Activity.this, "Lỗi tải danh mục: " + response.code(), Toast.LENGTH_SHORT).show();
                }
            }

            @Override
            public void onFailure(Call<List<Category>> call, Throwable t) {
                Toast.makeText(Admin_Product_Add_Activity.this, "Lỗi: " + t.getMessage(), Toast.LENGTH_SHORT).show();
            }
        });
    }
        ...
        call.enqueue(new Callback<List<Product>>() {
            ...
            public void onResponse(Call<List<Product>> call, Response<List<Product>> response) {
                if (response.isSuccessful()) {
                    ...
                    Toast.makeText(Admin_Product_Add_Activity.this, "Không thể tải danh sách sản phẩm: " + response.code(), Toast.LENGTH_SHORT).show();
                }
            }
            @Override
    ...
    private void addProduct() {
        ...
        call.enqueue(new Callback<Product>() {
            ...
            public void onResponse(Call<Product> call, Response<Product> response) {
                if (response.isSuccessful() && response.body() != null) {
                    ...
                    if (addedProduct.getId() <= 0) {
                        Toast.makeText(Admin_Product_Add_Activity.this, "Lỗi: ID sản phẩm không hợp lệ từ server", Toast.LENGTH_LONG).show();
                    } else {
                        productList.add(addedProduct);
                        productAdapter.notifyDataSetChanged();
                        clearInputs();
                        Toast.makeText(Admin_Product_Add_Activity.this, "Thêm sản phẩm thành công!", Toast.LENGTH_SHORT).show();
                    }
                } else {
                    try {
                        String errorBody = response.errorBody() != null ? response.errorBody().string() : "Unknown error";
                        Toast.makeText(Admin_Product_Add_Activity.this, "Lỗi khi thêm: " + errorBody + " (Mã: " + response.code() + ")", Toast.LENGTH_LONG).show();
                    } catch (IOException e) {
                        Toast.makeText(Admin_Product_Add_Activity.this, "Lỗi khi thêm: " + response.code(), Toast.LENGTH_SHORT).show();
                    }
                }
                isProcessing = false;
                btnAddProduct.setEnabled(true);
            ...
            public void onFailure(Call<Product> call, Throwable t) {
                Toast.makeText(Admin_Product_Add_Activity.this, "Lỗi kết nối: " + t.getMessage(), Toast.LENGTH_SHORT).show();
                isProcessing = false;
                btnAddProduct.setEnabled(true);
            }
        });
        ...
        call.enqueue(new Callback<Map<String, String>>() {
            ...
            public void onResponse(Call<Map<String, String>> call, Response<Map<String, String>> response) {
                if (response.isSuccessful()) {
                    ...
                    productAdapter.notifyDataSetChanged();
                    Toast.makeText(Admin_Product_Add_Activity.this, "Xóa thành công!", Toast.LENGTH_SHORT).show();
                } else {
                    try {
                        String errorBody = response.errorBody() != null ? response.errorBody().string() : "Unknown error";
                        Toast.makeText(Admin_Product_Add_Activity.this, "Lỗi khi xóa: " + errorBody + " (Mã: " + response.code() + ")", Toast.LENGTH_LONG).show();
                    } catch (IOException e) {
                        Toast.makeText(Admin_Product_Add_Activity.this, "Lỗi khi xóa: " + response.code(), Toast.LENGTH_SHORT).show();
                    }
                }
                isProcessing = false;
            }
